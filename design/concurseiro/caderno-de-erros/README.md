# Caderno de Erros — workspace do módulo

Módulo de **diagnóstico do raciocínio errado** — baseado no **错题本** (China) + **오답노트** (Coreia),
adaptado ao concurso brasileiro. **19/jun/2026.** Status: **definição/spec — pré-validação.**

> **Papel no produto (realinhado 19/jun):** entra em **duas formas** — (1) **seção *lite*** dentro do
> **Planner v1** (o módulo-âncora); (2) **caderno completo standalone** na **Fase 2** do ecossistema.
> O **v1 é o Planner**, não este módulo. Ver [`../conceitos/roadmap-faseado.md`](../conceitos/roadmap-faseado.md).

## Por que importa
- Preenche a **maior lacuna**: o digital diz *o quê* você errou; só o papel processa o *porquê*.
- **Complementa** (não compete com) QConcursos/TEC e Anki.
- Mais distintivo da linha — forte candidato a **módulo standalone** (Fase 2).

## Conteúdo desta pasta
- `spec.md` — especificação (estrutura da ficha, taxonomia, ritmo, formatos, físico).
- `user-stories.md` — JTBD e histórias (persona Rafael, full-time).
- `fontes.md` — proveniência (错题本/오답노트, ciência, não-colisão) com links.
- `benchmark-{china,coreia,japao}.md` — teardown visual de cadernos de erro reais (em pesquisa).
- `referencias/` — imagens (produtos/templates) por região.

## Material compartilhado (linkado, não duplicado)
- Método/tese: [`../conceitos/metodo-hub-analogico.md`](../conceitos/metodo-hub-analogico.md) §6.
- Definição do produto: [`../conceitos/definicao-do-produto.md`](../conceitos/definicao-do-produto.md).
- Roadmap faseado: [`../conceitos/roadmap-faseado.md`](../conceitos/roadmap-faseado.md).
- Pesquisa nativa do método: [`../pesquisa/fontes/uso-asia-china-coreia.md`](../pesquisa/fontes/uso-asia-china-coreia.md).

## Próximo
Benchmark visual (CN/KR/JP, em andamento) → wireframe da ficha → validação.
